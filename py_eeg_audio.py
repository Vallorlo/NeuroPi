...
def get_stage_number(stage_dir):
    ...
    try:
        ...
        return None
def process_trial_data(root_dir, verbose=True, create_visualizations=True, timestamp_padding=0.00025, generate_dataset=False):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--timestamp_padding', type=float, default=0.25, help='Padding for timestamps in seconds (default: 0.00025)')
    parser.add_argument('--create_visualizations', action='store_true', help='Create visualizations of processed EEG data')
    parser.add_argument('--generate_dataset', action='store_true',  help='Generate a combined dataset CSV')
    ...
    try:
        ...
        if which("ffmpeg") is None:
            print("\nWARNING: ffmpeg not found. Audio processing may not work correctly.")
            print("To install ffmpeg:")
            print("  - Windows: Download from https://ffmpeg.org/download.html or install with 'choco install ffmpeg'")
            print("  - macOS: Run 'brew install ffmpeg'")
            print("  - Linux: Run 'sudo apt-get install ffmpeg' or equivalent for your distribution")
            print("After installing, make sure ffmpeg is in your system PATH.")
    except:
        pass


    stats = process_trial_data(args.root_dir, verbose=args.verbose, create_visualizations=args.create_visualizations, timestamp_padding=args.timestamp_padding, generate_dataset=args.generate_dataset)
    ...
    print(f"Audio files processed: {stats['audio_processed']}")
    print(f"Timestamp files processed: {stats['timestamp_processed']}")
