...
class ModelTrainer:
    ...
    def preprocess_data(self):
        ...
        # 1 if any word event is True, 0 otherwise
        is_speech = df[self.word_event_columns].any(axis=1).astype(int).values
        
        # Extract word labels for speech segments
        word_labels = []
        for idx in range(len(df)):
            if is_speech[idx]:
                # Find which word event is True for this sample
                for event_col in self.word_event_columns:
                    if df.loc[idx, event_col]:
                        word = event_col.replace('_event', '')
                        word_labels.append(word)
                        break
            else:
                word_labels.append('sil')  # Silence label
    ...
    def __init__(self, model_path):
        ...
        try:
            self.speech_model.load_state_dict(torch.load(
                ...
                map_location=torch.device('cpu')  # Ensure model can be loaded without GPU
            ))
            ...
            self.pipeline = EEGSpeechPipeline(
                self.speech_model, self.word_model, self.scaler, self.words, self.sequence_length
            )
        except Exception as e:
            import traceback
            traceback.print_exc()
